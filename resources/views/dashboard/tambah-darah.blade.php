                    ...
                    <div class="col">
                        @if ($isFinished)
                            <button type="button" class="btn btn-danger" data-toggle="modal"
                                data-target="#exampleModal1">Form Menstruasi</button>
                        @endif
                        @if ($isNotFinishedMenstruation)
                            <button type="button" class="btn btn-primary" data-toggle="modal"
                                data-target="#exampleModal2">Tambah Data Menstruasi</button>
                        @endif
                        @if ($isNotFinishedNoMenstruation)
                            <button type="button" class="btn btn-warning" data-toggle="modal"
                                data-target="#exampleModal3">Tambah Data Tidak Menstruasi</button>
                        @endif
                    </div>
                </div>
                <div class="row mt-3">
                    ...
                    @foreach ($menstruations as $menstruation)
                        @if ($menstruation->frekuensi_tablet == 'Tidak' && !$printBadge)
                            @php $printBadge = true; @endphp
                            <span class="badge badge-warning">
                                Tidak diperkenankan untuk tidak mengonsumsi tablet tambah darah baik selama menstruasi
                                maupun tidak.<br>
                                Karena disaat menstruasi tubuh akan kehilangan zat besi sebesar ± 1,3mg/hari sehingga
                                dibutuhkan tablet tambah darah yang dapat meningkatkan kadar Hemoglobin.
                            </span>
                        @endif
                    @endforeach
                </div>
                <div class="row mt-3">
                    ...
                    @if ($isNotFinishedMenstruation)
                        <form method="post" action="{{ route('finish.menstruation') }}" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" value="{{ $latestMenstruation ? $latestMenstruation->id : 'null' }}"
                                name="menstruation_id">
                    ...
                    <form method="post" action="{{ route('store.tambah.darah') }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="menstruasi" name="menstruasi" required>
                                <option value="" {{ old('menstruasi') == '' ? 'selected' : '' }}>Pilih Jawaban
                                </option>
                        ...
                        <div class="form-group" id="hari_menstruasi-group" style="display: none;">
                            <label for="hari_menstruasi"
                                class="form-label @error('hari_menstruasi') is-invalid @enderror">Hari Menstruasi</label>
                            <input type="number" class="form-control" id="hari_menstruasi" name="hari_menstruasi"
                                placeholder="Input Hari Menstruasi Ke-" value="{{ old('hari_menstruasi') }}">
                            @error('hari_menstruasi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            <label for="frekuensi_tablet" id="frekuensi_tablet_label">Apakah anda mengonsumsi tablet
                                tambah darah?</label>
                            <select class="form-control" id="frekuensi_tablet" name="frekuensi_tablet" required>
                                <option value="" {{ old('frekuensi_tablet') == '' ? 'selected' : '' }}>Pilih Jawaban
                                </option>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="post" action="{{ route('store.menstruation') }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label for="hari" class="form-label @error('hari') is-invalid @enderror">Hari Menstruasi
                                Ke-</label>
                            <input type="number" class="form-control" id="hari" name="hari"
                                placeholder="Input Hari Menstruasi Ke-" value="{{ old('hari') }}">
                            @error('hari')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control" id="isKonsumsi" name="isKonsumsi" required>
                                <option value="" {{ old('isKonsumsi') == '' ? 'selected' : '' }}>Pilih Jawaban
                                </option>
                                <option value="Ya">Ya</option>
                                <option value="Tidak">Tidak</option>
                            </select>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="post" action="{{ route('store.no.menstruation') }}" enctype="multipart/form-data">
                        ...
                        <input type="hidden" value="{{ Auth::user()->id }}" name="user_id">
                        <div class="form-group">
                            <label for="minggu" class="form-label @error('minggu') is-invalid @enderror">Minggu
                                Ke-</label>
                            <input type="number" class="form-control" id="minggu" name="minggu"
                                placeholder="Input Minggu Ke-" value="{{ old('minggu') }}">
                            @error('minggu')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function toggleInputs() {
                if (menstruasiSelect.value === 'Ya') {
                    ...
                    frekuensiTabletLabel.textContent =
                        'Apakah anda konsumsi tablet tambah darah selama minggu ini?';
                }
            }
